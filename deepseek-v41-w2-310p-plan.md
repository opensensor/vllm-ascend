...
**Wave progress (2026-09-16):** E2.1 ✅ 2bee1bdd4 · E2.2 ✅ 7ce364b68 (MLA latent KV, ~85× smaller than MHA) · E3.1 ✅ e4cd6097a (MLA, parity 1e-9) · E3.2 ✅ 4311e2e8c (indexer ratio {0,1,2}) · E3.3 ✅ 777996d65 (MoE→E1.3 W2 method, Triton op→addcmul) · E2.3 ✅ 838ea0b6a (Engram host ~W4, hash+gather+gated proj) · E3.4 ✅ d157568f5 (W2 weight-map; **real-artifact placement = 36.95 GiB/chip < 40 GiB budget, imbalance 0, Engram 92.42 GiB host once**). ALL 6 components done; full deepseek_w2 suite 377 passed. E4.1 ✅ 0143d36a4 — **DeepSeek V4.1 boots on CPU with dummy weights** (hooks wired: MoE→W2, Engram@[1,14], MLA+ratio-{1,2} indexer; KV-group report matches E2.2; deterministic; Triton-free; load_weights maps the W2 artifact into the E1.3 layout, FP16 by name, Engram→host, rejections fire). Host-eager assembly since the shipped V4 base isn't CPU-importable. Full deepseek_w2 suite 392 passed. E4.2 ✅ 81ce10859 (host MTP-3 drafter adapting deepseek_v4 MTP; reuses the E4.1 eager decoder layer per step; DSpark spec target [37,38,39]/128-top3/block-5/rank-256 recorded; Triton-free; registry repointed model→mtp). **DeepSeek E2–E4 COMPLETE — full deepseek_w2 suite 406 passed. DeepSeek is bring-up-ready** — remaining is on-device (D-wave) validation of the npu_* MLA/indexer/W2 kernels + the device hyper-connection path (host eager stand-ins today). Each component built as its own module; model.py hooks (`_swap_moe_to_w2`/`_inject_engram`/`_override_mla_indexer`) filled by E4.1.
