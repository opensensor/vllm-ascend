...
// Elements per 32B copy block, the granularity 310P's DataCopy works in.
template <typename T>
__aicore__ inline uint32_t ElemsPerBlock()
{
    if constexpr (std::is_same_v<T, int4b_t>) {
        return ONE_BLK_SIZE * 2;
    } else {
...
{
#ifdef RMS_NORM_DQ_NO_DATA_COPY_PAD
    const uint32_t strideBlk = (ubAligned) ? 1 : 0;
    if constexpr (is_same<R<T>, AscendC::LocalTensor<T>>::value) {
        CompatCopyGm2Ub(dst, src, len, count, strideBlk, ElemsPerBlock<T>());
    } else {
        CompatCopyUb2Gm(dst, src, len, count, strideBlk, ElemsPerBlock<T>());
    }
#else
...
#ifdef RMS_NORM_DQ_NO_DATA_COPY_PAD
    if constexpr (is_same<R<T>, AscendC::LocalTensor<T>>::value) {
        CompatCopyGm2Ub(dst, src, len, count, ubAligned, ElemsPerBlock<T>());
    } else {
        CompatCopyUb2Gm(dst, src, len, count, ubAligned, ElemsPerBlock<T>());
    }
#else
    DataCopyExtParams copyParams;
    copyParams.blockCount = count;
