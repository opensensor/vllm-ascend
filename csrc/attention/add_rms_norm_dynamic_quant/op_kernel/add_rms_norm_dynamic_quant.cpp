...
/*!
 * \file add_add_rms_norm_dynamic_quant.cpp
 * \brief
 */
...
extern "C" __global__ __aicore__ void add_rms_norm_dynamic_quant(
    GM_ADDR x, GM_ADDR residual, GM_ADDR gamma, GM_ADDR smooth1, GM_ADDR smooth2, GM_ADDR beta, GM_ADDR y1, GM_ADDR y2,
    GM_ADDR residualOut, GM_ADDR outScale1, GM_ADDR outScale2, GM_ADDR workspace, GM_ADDR tiling)
{
    TPipe pipe;
...
#define INIT_AND_PROCESS                                                                                \
    op.Init(x, residual, gamma, smooth1, smooth2, beta, y1, y2, residualOut, outScale1, outScale2, usrWorkspace, &tilingData); \
    op.Process()
    if (TILING_KEY_IS(0)) {
        // 0 Tiling, Do Nothing.
