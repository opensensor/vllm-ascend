...
// One 32B GM store holds eight fp32 per-token scales.
constexpr uint64_t ROWS_PER_SCALE_BLOCK = BLOCK_SIZE / sizeof(float);
...
bool AddRmsNormDynamicQuantTilingHelper::DoBlockTiling()
{
    ...
    if (this->isAscend310P_) {
        // Give every core a whole number of 32B scale blocks. Only the last core
        // is then left ragged, and its surplus runs off the end of the scale
        // tensor -- which the host over-allocates to a multiple of 8 rows --
        // rather than into rows another core owns.
        this->firstDimPerCore_ =
            CeilDiv(this->firstDimPerCore_, ROWS_PER_SCALE_BLOCK) * ROWS_PER_SCALE_BLOCK;
        this->useCore_ = CeilDiv(this->numFirstDim_, this->firstDimPerCore_);
    }
...
bool AddRmsNormDynamicQuantTilingHelper::InitializePlatformInfo()
{
    ...
    this->isAscend310P_ = (ascendcPlatform.GetSocVersion() == platform_ascendc::SocVersion::ASCEND310P);
...
bool AddRmsNormDynamicQuantTilingHelper::CalculateShapeParameters()
{
    ...
    // 310P has no DataCopyPad (see op_kernel/compat_310p.h): the DataCopy
    // emulation only reproduces it exactly when a row is a whole number of 32B
    // blocks, which is what an unpadded last dim means here.
    OPS_CHECK(
        this->isAscend310P_ && (this->numLastDimAligned_ != this->numLastDim_),
        OPS_LOG_E(
            context_->GetNodeName(), "310P needs the last dim 32-element aligned, got %lu.",
            this->numLastDim_),
        return false);
...
bool AddRmsNormDynamicQuantTilingHelper::CheckUbNormalTiling()
{
    ...
    bool ret = (rowStep >= 1);
    if (this->isAscend310P_) {
        // Each loop flushes rowStep scales, and that store has to start and end
        // on a 32B boundary. If UB cannot fund a whole 8-row block, fall through
        // to SingleRow, which flushes scales 128 rows at a time.
        rowStep = rowStep / static_cast<int64_t>(ROWS_PER_SCALE_BLOCK) *
                  static_cast<int64_t>(ROWS_PER_SCALE_BLOCK);
        ret = (rowStep >= static_cast<int64_t>(ROWS_PER_SCALE_BLOCK));
    }
    OPS_LOG_I(
...
bool AddRmsNormDynamicQuantTilingHelper::CheckUbSliceDTiling()
{
    ...
    OPS_CHECK(
        this->isAscend310P_,
        OPS_LOG_E(this->context_->GetNodeName(), "SliceD tiling is not supported on 310P."), return false);
