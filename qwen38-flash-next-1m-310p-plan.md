...
- **status**: Completed
- **log**: 2026-09-16 (commit 652107986) — Additive `Qwen4ExpPlacementSimulator` (+`TensorGroup`/`ShardPolicy`/`PlacementLedger`/`DoubleInstantiationError`) in `sharded_state_loader_310p.py`; existing `ShardedStateLoader310` untouched; integrates T0.5 `MemoryAccountant`. **Manifest-derived per-chip non-PLE = 32.01 GiB/chip** (128.06 GiB agg /4) vs 31.88 target → **+0.42% drift** (tol 3%); root cause = quant scales aggregate ~0.70 GiB (F32 [out,1]×73,728) vs the ~0.19 GB first attributed (folded into the runtime-requirements doc; T8.3 shifts ~0.13 GiB/chip, no pass/fail change). PLE 95.43 GiB host-resident, excluded from device, counted once. EP4==TP4 (each rank exactly 1/4 of the 512-expert bank, imbalance 0). Invariants proven: no double-instantiation (`PlacementLedger` union==whole; 222,746 tensors reconcile to manifest total + export bytes <0.5%); no rank holds full bank (each rank EXPERT_W8A8 ≈ total/4); bounded RSS (tracemalloc peak <64 MB while accounting >200 GiB — metadata walk only). 13 UTs; ruff clean.
- **files edited/created**: `vllm_ascend/_310p/sharded_state_loader_310p.py` (+simulator, additive), `tests/ut/qwen38_1m/test_shard_placement.py` (new)
