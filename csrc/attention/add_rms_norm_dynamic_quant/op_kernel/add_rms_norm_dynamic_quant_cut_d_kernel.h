...
/*!
 * \file add_add_rms_norm_dynamic_quant_cut_d_kernel.h
 * \brief
 */
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int32_t rowIdx = 0; rowIdx < this->rowWork; ++rowIdx) {
            ...
            for (int32_t colIdx = 0; colIdx < this->lastDimLoopNum; ++colIdx) {
                ...
                this->localSum += ReduceSquareSumSlice(baseGmOffset, rowGmOffset, this->lastDimSliceLen);
                PipeBarrier<PIPE_V>();
...

            {
                ...
                this->localSum += ReduceSquareSumSlice(baseGmOffset, rowGmOffset, this->lastDimSliceLenTail);
                PipeBarrier<PIPE_V>();
            }
...
private:
    ...
    __aicore__ inline void CopyInX(int32_t baseGmOffset, int32_t rowGmOffset, int32_t elementCount)
    {
        ...
        DataCopyEx(x1x2LocalIn[0], this->xGm[baseGmOffset + rowGmOffset], elementCount);
        DataCopyEx(x1x2LocalIn[elementCount], this->residualGm[baseGmOffset + rowGmOffset], elementCount);
    ...
    __aicore__ inline float ReduceSquareSumSlice(int32_t baseGmOffset, int32_t rowGmOffset, int32_t elementCount)
    {
        LocalTensor<float> xLocalFp32 = xBufFp32.Get<float>();
        ...
        // Emit the residual sum before the square-sum overwrites yLocalFp32.
        LocalTensor<T> residualOutLocal = yBufFp32.Get<T>();
        PipeBarrier<PIPE_V>();
        Cast(residualOutLocal, xLocalFp32, RoundMode::CAST_NONE, elementCount);
        PipeBarrier<PIPE_V>();
        DataCopyEx(this->residualOutGm[baseGmOffset + rowGmOffset], residualOutLocal, elementCount);
        PipeBarrier<PIPE_V>();
