...
### G4: Triton-free 310P KDA linear attention — COMPLETED (2026-09-16, commit 34fd9cb3f)
`vllm_ascend/models/glm5next_w2/kda.py` + `tests/ut/glm_w2/test_glm5next_w2_kda.py` (18 UTs). Eager gated-delta recurrence (short causal conv1d width-4 + SiLU → safe gate `-5*sigmoid(exp(A_log)*(g+dt_bias))` → per-head state S[128,128] delta-rule with per-head scalar beta → sigmoid-gated RMSNorm → o_proj), numerically matching the G-ref oracle `tools/glm_w2/kda_reference.py` within tolerance. No triton / `ops.triton` on the import path (guarded torch_npu for CPU host); package grep-gate 11/11, kda_reference 22/22 unregressed; ruff clean. Lazy `_KDA_EXPORTS` added to package init. (Agent was rate-limited mid-finish after GREEN; orchestrator fixed 2 ruff SIM114 + 1 E501 in the test grep-gate helper and committed with the Co-Authored-By trailer.)

### G6: 310P W2 routed-MoE + weight mapping — COMPLETED (2026-09-16, commit 0925a29a5)
`vllm_ascend/models/glm5next_w2/{moe.py,weight_mapping.py}` + `tests/ut/glm_w2/test_glm5next_w2_moe.py` (25 UTs) + package exports. `Glm5NextW2MoE`: GLM sigmoid `noaux_tc` top-8/288 host router (`glm_route_topk`, reduces bit-for-bit to the DeepSeek `route_topk_w2` anchor in the softmax/no-bias/n_group=1 config) + `norm_topk_prob`; routed experts through the REUSED E1.3 `AscendW2DynamicFusedMoEMethod310` (active-expert W2→INT8 unpack, method resolved lazily — needs torch_npu, so CPU tests use an injected method); eager `routed*2.5 + FP16 shared` combine (`eager_moe_combine` reused); FFN-side multi-head hyper-connection `hc_pre`/`hc_post`/`hc_expand`/`hc_contract` (Triton-free re-expression of `mhc`). `weight_mapping.py` (reuses DeepSeek `TensorMeta`/error classes/W2 constants): GLM-named experts `model.language_model.layers.{L}.mlp.experts.{E}.{gate,up,down}_proj_{codes,scale}` → fused `w13`/`w2` slots (gate→off0, up→off inter, down→w2); `first_k_dense_replace=3` dense layers excluded, MTP layer included; **validated vs the real 97 GB artifact: 43 MoE blocks, 74304 expert tensors, converted shapes exact** (gate/up codes [2048,1024] scale [64,128]; down codes [4096,512] scale [128,64]). No triton; package grep-gate + dsa + kda unregressed (37 passing); ruff clean. (moe.py authored by the rate-limited G6 agent; orchestrator built weight_mapping + tests + exports, removed an unused import, committed with trailer.)
