...
/*!
 * \file add_add_rms_norm_dynamic_quant_normal_kernel.h
 * \brief
 */
...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int32_t rowIdx = 0; rowIdx < rowMoveCnt - 1; ++rowIdx) {
            CopyInXAndResidual(gmOffset, this->rowStep, elementCount);
            ComputeRmsNorm(gmOffset, this->rowStep, elementCount, gammaLocal);
        ...
        }
        {
            ...
            CopyInXAndResidual(gmOffset, this->rowTail_, elementCount);
            ComputeRmsNorm(gmOffset, this->rowTail_, elementCount, gammaLocal);
...
private:
    __aicore__ inline void CopyInXAndResidual(int32_t gmOffset, int32_t rowCount, int32_t elementCount)
    {
        LocalTensor<T> x1x2LocalIn = inRowsQue.template AllocTensor<T>();
        DataCopyExStride(x1x2LocalIn[0], this->residualGm[gmOffset], this->numLastDim, rowCount, this->ubAligned);
        DataCopyExStride(x1x2LocalIn[elementCount], this->xGm[gmOffset], this->numLastDim, rowCount, this->ubAligned);
    ...
    __aicore__ inline void ComputeRmsNorm(int32_t gmOffset, int32_t nums, int32_t elementCount, LocalTensor<T>& gammaLocal)
    {
        LocalTensor<float> xLocalFp32 = xBufFp32.Get<float>(); // xLocalFp32 <-- x + residual
        ...
        Add(xLocalFp32, xLocalFp32, yLocalFp32, elementCount); // xLocalFp32 <- x + residual

        // Emit the residual sum (x + residual) before the norm overwrites xLocalFp32.
        LocalTensor<T> residualOutLocal = yBufFp32.Get<T>();
        PipeBarrier<PIPE_ALL>();
        Cast(residualOutLocal, xLocalFp32, RoundMode::CAST_NONE, elementCount);
        PipeBarrier<PIPE_ALL>();
        DataCopyExStride(
            this->residualOutGm[gmOffset], residualOutLocal, this->numLastDim, nums, this->ubAligned);
        PipeBarrier<PIPE_ALL>();
