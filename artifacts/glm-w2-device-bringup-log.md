...
## CORRECTNESS DEBUG cont'd (2026-09-20)
- **bf16 ruled out.** 310P does bf16 *compute* on isolated ND tensors, but the model op surface is broadly bf16-unsupported on CANN 26.0.rc1: Cube `aclnnMatmul` (fp16/fp32 only, esp. mixed fp32xbf16), HCCL AllReduce (EI0003), ND->NZ TransData (EZ3002), and pointwise `Muls`/`Clamp`/`InplaceOne`/`MaskedFill`/`embedding`/`swiglu`. Full bf16 needs fp32-shimming the whole op surface = no perf/correctness benefit. Correct arch = **fp16 storage + fp32 accumulation** (already the design).
- **KDA recurrence is ALREADY fp32** (`kda.py compute_dtype=cast_site('kda_accumulation')=float32`); its output is genuinely tiny by construction (q scaled by head_dim**-0.5), NOT fp16 underflow. The eps-hack is a valid scale-invariant renorm.
- **Bug #4 mHC input-RMSNorm never applied (FIXED, commit 33d3e5636).** The reinstating patch was behind `if HAS_TRITON:` in patch/worker/__init__.py; 310P has no Triton -> mhc_pre returned an un-normalized gated residual sum -> mlp_in L2 grew 3.7->1510 -> quadratic SwiGLU blowup -> late-layer MoE explosion. Fix: `patch/worker/patch_mhc_norm.py` (Triton-free) imported unconditionally. mlp_in now == ||gamma||2.
- **REMAINING (unsolved): near-uniform logits = component-DIRECTION bug.** After #1-#4, magnitudes are correct but logits are near-uniform (entropy ~10.3 nats, eff-rank ~33k/155k, top1-top2 gap ~0) and nondeterministic at temp0. NOT an fp16 limit (repro on fp32 eager MoE). lm_head + final norm ruled out (healthy). Nondeterminism seed = tiny layer-0 KDA reduction amplified through depth (symptom). Ranked suspects: (1) KDA direction (34 layers), (2) DSA/MLA on 310P (11 layers, `AscendMLABackend310` unverified), (3) W2 expert/routing direction. **Localizing requires a 910/GPU GOLDEN REFERENCE (per-layer cosine diff of KDA/MLA/expert outputs) — not available on the 310P box.** Isolated component parity tests pass, so the bug is in assembled/wired behavior a full-model reference would expose.
- **State: committed** (branch glm5next-w2-310p-bringup, commits 9dfcd1285 + 33d3e5636). Fit + ~17x Cube kernel + 4 correctness fixes; output = real words, not yet coherent. Firmware-audit workstream uncommitted.
